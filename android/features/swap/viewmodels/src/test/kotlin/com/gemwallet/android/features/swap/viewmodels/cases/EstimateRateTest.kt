...
class EstimateRateTest {
    ...
    fun `estimateRate should calculate correct rates for ETH to USDC swap`() {
        ...
        val expected = SwapProperty.Rate(
            forward = "1 ETH ≈ 2,000.00 USDC",
            reverse = "1 USDC ≈ 0.0005 ETH"
        )
        
        assertRatesEqual(expected, result)
    }
    ...
    private fun assertRatesEqual(expected: SwapProperty.Rate, actual: SwapProperty.Rate?) {
        assertNotNull(actual)
        assertEquals(expected.forward, actual?.forward)
